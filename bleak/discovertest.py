...
async def run(address, debug=False):
    ...
    # async with device as client:
        ...
        for service in client.services:
            ...
            # rw_charac = "00002902-0000-1000-8000-00805f9b34fb" # w33
            ...
            while True:
                ...
                try:
                    data = bytearray.fromhex(data.decode('utf-8'))
                    # data = [0, 2, 100]
                    print("data:", data)
                    ...
                    await client.write_gatt_char(rw_charac, data, True)
                    # await client.write_gatt_char(rw_charac, data, False)
                    print("sent:", data)
                except:
            ...
            # await asyncio.sleep(1.0)  # Sleeping just to make sure the response is not missed...

            # for x in range(255):
            #     # print(bytes(x))
            #     # print(x.to_bytes(2, byteorder='big'))
            #     print(f"{x:02x}")
            #     # await client.write_gatt_char(rw_charac, x.to_bytes(2), response=True)
            #     await client.write_gatt_char(rw_charac, x.to_bytes(2, byteorder='big'), response=True)
            #     value = await client.read_gatt_descriptor(13)
            #     print('value:', value)
            #     await asyncio.sleep(1.0)  # Sleeping just to make sure the response is not missed...

            # list = ['000000000000000000000', 'hello', 'play']
            inputlist = [[0x00, 0xff], 'hello', 'play']
            # list = ['02, 02, 01, 03, 1e, 5d', 'hello', 'play']
            for x in inputlist:
                print(x)
                await client.write_gatt_char(rw_charac, bytearray(x), response=True)
                await asyncio.sleep(.5)  # Sleeping just to make sure the response is not missed...
                value = await client.read_gatt_descriptor(13)
                print('value:', value)
